# app.py (Finale Version - Lädt Modelle aus der DB)
...
def load_artifacts_from_db():
    """Lädt alle Modelle und Scaler aus der Datenbank."""
    global btc_model, gold_model, btc_scaler, gold_scaler
    try:
        with app.app_context():
            artifacts = TrainedModel.query.all()
            if not artifacts:
                print("WARNUNG: Keine Modelle in der DB gefunden. Signale werden nicht funktionieren.")
                return

            for artifact in artifacts:
                obj = pickle.loads(artifact.data)
                if artifact.name == 'btc_model': btc_model = obj
                elif artifact.name == 'gold_model': gold_model = obj
                elif artifact.name == 'btc_scaler': btc_scaler = obj
                elif artifact.name == 'gold_scaler': gold_scaler = obj
...
# --- API-Routen ---
# Alle Routen (/get_signals, etc.) bleiben exakt so wie in der letzten funktionierenden Version
# Sie werden jetzt die globalen Modell-Variablen verwenden, die aus der DB geladen wurden.
@app.route('/')
def home():
...
def get_signals():
    ...
    else: error_msg += "BTC Modell/Scaler nicht geladen. "; bitcoin_data={"signal_type":"Fehler"}
