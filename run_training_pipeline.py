# run_training_pipeline.py (Finale Version mit Bugfix für Benachrichtigungen)
...
def save_artifact_to_db(name, artifact):
    ...
    db.session.commit()
    print(f"'{name}' in der DB gespeichert/aktualisiert.")
...
def run_full_pipeline():
    ...
    with app.app_context():
        # KORREKTUR: Firebase wird hier initialisiert, direkt bevor es gebraucht wird.
        if not firebase_admin._apps:
            print("Initialisiere Firebase Admin SDK innerhalb des App-Kontexts...")
            try:
                # Wir verwenden nur die Umgebungsvariable, da der Cron Job nur auf Render läuft
                ...
                if cred_str:
                    ...
                    print("FEHLER: FIREBASE_SERVICE_ACCOUNT_JSON Umgebungsvariable nicht gefunden.")
                    return # Pipeline abbrechen
            except Exception as e:
                print(f"FEHLER bei Firebase-Initialisierung: {e}"); return

        db.create_all()
        # ... (Rest der Logik bleibt exakt gleich wie vorher) ...
        ...
        for asset_name, details in asset_map.items():
            ...
            if new_signal != last_signal and device_tokens:
                ...
                title = f"Neues Krypto-Helfer Signal: {asset_name}"; body = f"Das Signal für {asset_name} ist jetzt: {new_signal.upper()}"
